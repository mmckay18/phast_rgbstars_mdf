...
def make_optical_photmetry_from_hst_dataproduct(photmetry_catalog_filepath, output_csv_filepath):
    """
    ...
    if photmetry_catalog_filepath.endswith('.fits'):
        ...
        ra, dec, f475w_vega, f475w_snr, f475w_crowd, f475w_sharp, f475w_flag, f814w_vega, f814w_snr, f814w_crowd, f814w_sharp, f814w_flag = ds.evaluate(['RA', 
                                                                            'DEC', 
                                                                            'F475W_VEGA', 'F475W_SNR', 'F475W_CROWD', 'F475W_SHARP', 'F475W_GST_FLAG', 
                                                                            'F814W_VEGA', 'F814W_SNR', 'F814W_CROWD', 'F814W_SHARP', 'F814W_GST_FLAG'],
        ...
        df = pd.DataFrame({'ra': ra, 'dec': dec, 'f475w_vega': f475w_vega, 'f475w_snr': f475w_snr, 'f475w_crowd': f475w_crowd, 'f475w_sharp': f475w_sharp, 'f475w_flag': f475w_flag, 'f814w_vega': f814w_vega, 'f814w_snr': f814w_snr, 'f814w_crowd': f814w_crowd, 'f814w_sharp': f814w_sharp, 'f814w_flag': f814w_flag})
        # df = df.loc[((df['f475w_sharp']+df['f814w_sharp'])**2 <= 0.075)]
        print('Saving as CSV file...')
        df.to_csv('/astro/users/mmckay18/analysis_routine_PHAST/DATA/phat_f475W_f814W_table.csv', index=False)
...
def reduce_optical_photmetry_table(photometry_fits_filepath, output_filepath):
    ...
    catalog_df = df.loc[(df['f475w_flag'] == 0.0) & (df['f814w_flag'] == 0.0)
    ...
    catalog_df['f475w-f814w_ecorr'] = catalog_df['f475w_vega_ecorr']-catalog_df['f814w_vega_ecorr']
    catalog_df = catalog_df.loc[(catalog_df['f814w_vega_ecorr'] <= 23.0)] # Apply after foreground reddening correction
...
def catalog_linear_interpolation(catalog_csv_filepath, isochrone_csv_filepath, output_filepath, savefile=True):
    """
    ...
    print('Dropping fill values if any')
    ...
    if savefile is True:
        print(f'Saving the photometry catalog with interpolated values as {output_filepath}')
